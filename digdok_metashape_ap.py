...
def vars(uuid):
  ...
  # findme suggestion: possible to use named array?
  if mode == "db":
    query = (
      "SELECT settings.*"
  ...
  #findme todo: move this setting_group out into json files to make offline handling of presets easier?
  # If standalone mode, set vars here 
    ...
    ## Error correction Percentages
    # findme todo: review values  prefer combined point removal of all 3 selections to not be more than 30%
    RU_Percent = 20
    PA_Percent = 20
    RE_Percent = 20
    ...
    ## CRS
    # findme todo: move out? defualt to local? CRS can be set with GCP import
    crs = 32630
    ...
    # findme todo: add dense cloud creation
...
# finddme suggestion:
  # replace DB with a folder and json solution easier to standup an standalone instance?
...
# findme todo:  add support for iq in masked areas only
...
##### findme todo: add load precalibrated lens data

##### findme todo: add focus group setup (should we group in folders, or feed in start numbers?)
...
def poptargets():
  ...
  # findme somment: is it best to keep all source at the file level rather than stored in the DB?
...
# findme note: must be sure not include scalebars before any realignment stages if using iterative aligment
def add_scalebars():

  scalebarfile = path + "/scalebars.csv"     # Path to the folder and target file name to write and read.

  # findme todo: add uuid/chunk id to scalebar targets to allow for reuse scalebars for eks. in two sides of an object model while still permitting aut import of scales.
  # If target data in database, get targets and make csv
  if mode == "db":
    query = (
...
#findme suggestion: better to remove markers so don't build scalebars on bad markers? Could be good to save them out afterward anyway.
def uncheckmarkers():
  target_list = []
...
# findme todo: is it possible to allow default behaviour here?  Forcing fit of too many parameters can be derimental
#Optimize alignemnts
def optimizealignments():

  # findme todo: check if calibrated lens and iterative alignment is being used.
    # if it is, no optomize, or optomize with no lens paramater adjustment.
...
# findme comment: They've renamed things in version 2.0.  densecloud= point cloud. not sure if this will upset the code here?
#--------------------------------------------------------------------------------
...
# - Check whitch filter level is better for objects. 
    #findme comment: ultra and mild best settings (most of the time) for objects.
...
# findme todo: insert inspection stage?
# findme todo: insert iterative mesh reconstruction for mesh masking between iterative alignment procedures
# findme todo: auto align halves on features and merge halves (for objects)
...
# findme todo: DoF image masking for textures (plus export)

# findme todo: optional swap image source to texture from different data
   # i.e. process on full bit depth with dng, then swap to color-corrected jxl/jpg for texture
...
def export():
  ...
  # findme suggestion: redevelop as module for the cook, restrict this script just for producing the 'master file'
  # Create Nexus files
...
def run(runmode):
  ...
  if align_bool:
    ...
    if get_status(uuid, "aligning") in ["done", "skip"]:
      ...
      try:
        # findme todo: implement iterative alignment, pass vars to align direct instead of globa (or update global vars here?)
